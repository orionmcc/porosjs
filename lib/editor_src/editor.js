...
function onCollectionRecords (ctx, next) {
  showRecordsSubMenu(ctx.params.collection, ctx.params.records);
}
function onEditRecord (ctx, next) {
  editRecord(ctx.params.data, ctx.params.meta);
}
function onCreateRecord (ctx, next) {
  createRecord(ctx.params.meta);
}
...
window.addEventListener('load', function(){
  ...
  page('/poros-collections/:collection/new', preRoute, loadCollectionMeta, onCreateRecord);
  page('/poros-collections/:collection/:record', preRoute, loadCollectionMeta, loadRecord, onEditRecord);
...
function showRecordsSubMenu (collection, records) {
  ...
  if (records) {
    for (var i = 0; i < records.length; i++) {
      menuHTML += "<li>";
      menuHTML += "<a class='poros-actionable' href='/poros-collections/"+collection+"/"+records[i].id+"'>"+records[i].name+"</a>";
    }
  }
...
function createRecord(collection) {
  ...
  if (pid) transitionSubmenu(function() { editRecord({pid: pid, id: null}, collection); });
...
function editRecord(data, collection, isNew) {
  ...
  for (var i = 0; i < collection.fields.length; i++) {
    ...
    //TODO: Handle by type
    menuHTML += "<input id='data-"+collection.fields[i].id+"' type='text' name='"+collection.fields[i].id+"' value='"+(data[collection.fields[i].id] || '')+"'/>";
  }
  menuHTML += "<li><button id='poros-button-okay' />UPDATE</button><button id='poros-button-cancel' />Cancel</button>";
  if (!isNew) menuHTML += "<li><button id='poros-button-delete' />DELETE PAGE</button>";
  menuHTML += "</ul>";
  ...
  var subMenuUpdate = function() {
    var id = document.getElementById('data-id').value || null;
    var pid = document.getElementById('data-pid').value || null;

    if (pid !== null  && id !== null) {
      //submit
      var url = '/api/collections/'+collection.id;
      if (id !== "null") url += '/' + id;
      var data = 'pid='+pid;
      for (var i = 0; i < collection.fields.length; i++) {
        data += "&"+collection.fields[i].id+"="+document.getElementById('data-'+collection.fields[i].id).value || '';
      }
...
function editPage(data, templates) {
  ...
  menuHTML += "</select>";
  menuHTML += "<li>Index Page <input id='data-index' type='checkbox'"
  if (data.default === "true") menuHTML += "CHECKED"
  menuHTML += " />";
  menuHTML += "<li><button id='poros-button-okay' />UPDATE</button><button id='poros-button-cancel' />Cancel</button>";
  ...
  var subMenuUpdate = function() {
    ...
    var index = document.getElementById('data-index').checked == true;
    ...
    if (slug && template) {
      ...
      enqueueData(url,
        "POST",
        "slug="+slug+"&template="+template+"&access="+access+"&index="+index,
        function() { subMenuBack(); }
      );
    }
  }
