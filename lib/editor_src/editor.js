...
function editPage(data, templates) {
  ...
  var subMenuUpdate = function() {
    ...
    if (slug && template) {
      ...
      enqueueData(url, "slug="+slug+"&template="+template+"&access="+access);
...
function showUsersSubMenu (users) {
  ...
  menuHTML += "<ul>";
  menuHTML += "<li><span class='poros-actionable' onClick='createUser()'>+ CREATE USER</span>";
  if (users) {
    for (var i = 0; i < users.length; i++) {
      menuHTML += "<li>";
      menuHTML += "<span class='poros-actionable' onClick='transitionSubmenu(function() {editUser("+JSON.stringify(users[i])+")})'>/"+users[i].username+"</span>";
    }
  }
  menuHTML += "</ul>";
