...
function onEditRecord (ctx, next) {
  const currentRevision = ctx.params.revisions.length > 0 ? ctx.params.revisions[0].restoreId : null
  showRecordsSubMenu(ctx.params.collection, ctx.params.records.map(function(d, idx) { return Object.assign({}, d, { _idx: idx }); }  ));
  editRecord(ctx.params.data, ctx.params.meta, false, currentRevision);
}
function onCreateRecord (ctx, next) {
  createRecord(ctx.params.meta, ctx.params.records);
}
function onShowRevisions (ctx, next) {
  showRevisionsSubMenu(ctx.params.collection, ctx.params.record, ctx.params.revisions)
}
function onEditRevision (ctx, next) {
  showRevisionsSubMenu(ctx.params.collection, ctx.params.record, ctx.params.revisions)

  const revision = ctx.params.revisions.filter(function(revision) { return revision.restoreId === ctx.params.revisionId })
  if (revision.length) editRevision(revision[0], ctx.params.data, ctx.params.meta, ctx.params.record, ctx.params.revisionId, revision[0].restoreId === ctx.params.revisions[0].restoreId)
}
...
window.addEventListener('load', function(){
  ...
  page('/poros-collections/:collection/:record', preRoute, loadCollectionMeta, loadCollection, loadRecord, loadRecordRevisions, onEditRecord);
  page('/poros-collections/:collection/:record/revisions', preRoute, loadRecordRevisions, onShowRevisions); // Load the revisions datas
  page('/poros-collections/:collection/:record/revision/:revisionId', preRoute, loadCollectionMeta, loadRecord, loadRecordRevisions, onEditRevision); // Load the revision data
...
function editRecord(data, collection, isNew, currentRevision) {
  tinymce.remove('div');
  var supportRevisions = collection.options.revisionHistory;

  console.log('Show sub menu for '+data.pid);
  var menuHTML = "";
  menuHTML += "<h2>"+data.pid+"</h2>";
  if (supportRevisions) menuHTML += "<a href='/poros-collections/"+collection.id+"/"+data.pid+"/revisions'>Revision History " +(currentRevision ? "("+currentRevision+")" : "")+ "</a>";
...
function showUsersSubMenu (users) {
  ...
  //menuHTML += "<a class='poros-actionable' href='/poros-users/new'>new user</a>";
	menuHTML += "To add a new user use, yarn add:user on the cmd line."
  menuHTML += "</ul>";
...
function editUser(user) {
  ...
  menuHTML += "</select>";
  if (user.tfasecret)  menuHTML += "<li><img id='poros-tfa-qrcode' class='poros-hide' src='https://chart.googleapis.com/chart?chs=230x230&cht=qr&chl=otpauth%3A%2F%2Ftotp%2F"+user.appname+"%2F"+user.username+"%3Fsecret%3D"+user.tfasecret+"&choe=UTF-8' />";
  menuHTML += "<li><button id='poros-button-okay' />UPDATE</button><button id='poros-button-cancel' />Cancel</button>";
  menuHTML += "</ul>";
...
function showRevisionsSubMenu (collection, record, revisions) {
  var menuHTML = "";
  menuHTML += "<h2>"+record+" REVISIONS</h2>";

  menuHTML += "<ul>";
  if (revisions) {
    for (var i = 0; i < revisions.length; i++) {
      menuHTML += "<li>";
      menuHTML += "<a class='poros-actionable' href='/poros-collections/"+collection+"/"+record+"/revision/"+revisions[i].restoreId+"'>"+revisions[i].restoreId+"</a>";
    }
  }
  menuHTML += "</ul>";
  menuHTML += "<a class='poros-actionable' href='/poros-collections/"+collection+"/"+record+"'>BACK</a>";

  showSubMenu(menuHTML);

  var subMenuBack = function(){ page('/poros-collections/'+collection+"/"+record) }
  var subMenuUpdate = function() {}
...
function editRevision(data, currentVersion, collection, record, revisionId, isCurrentRevision) {

  var menuHTML = "";
  menuHTML += "<h2>"+revisionId+"</h2>";

  if (!isCurrentRevision) {
    menuHTML += "<p><button id='poros-button-view' />" + (window.localStorage.getItem('view') === 'diff' ? "<span>Object</span>" : "<span>Diff</span>") + "</button></p>";
  }

  menuHTML += "<div id='poros-dataview' "+ (window.localStorage.getItem('view') !== 'diff' && !isCurrentRevision ? "class='poros-editor-hidden-tab'" : "") +">";
  menuHTML += "<p>Object</p>";
  menuHTML += "<ul>";
  for (var i = 0; i < collection.fields.length; i++) {
    menuHTML += "<li>";
    menuHTML += collection.fields[i].id+"<br />";
    //TODO: Handle by type
    if (collection.fields[i].type == 'json') menuHTML += "<input disabled id='data-"+collection.fields[i].id+"' type='text' name='"+collection.fields[i].id+"' value='"+(JSON.stringify(data[collection.fields[i].id] || ''))+"'/>";
    if (collection.fields[i].type == 'readonly') menuHTML += "<input disabled id='data-"+collection.fields[i].id+"' type='text' name='"+collection.fields[i].id+"' value='"+(data[collection.fields[i].id] || '')+"'/>";
    if (collection.fields[i].type == 'text') menuHTML += "<input disabled id='data-"+collection.fields[i].id+"' type='text' name='"+collection.fields[i].id+"' value='"+(data[collection.fields[i].id] || '')+"'/>";
    if (collection.fields[i].type == 'richtext') {
      var fieldId = collection.fields[i].id;
     menuHTML += "<div id='data-"+fieldId+"'>"+(data[fieldId] || '')+"</div>";
      // setTimeout(function () {
    ...
    if (collection.fields[i].type == 'number') menuHTML += "<input disabled id='data-"+collection.fields[i].id+"' type='text' name='"+collection.fields[i].id+"' value='"+(data[collection.fields[i].id] || '')+"'/>";
    if (collection.fields[i].type == 'boolean') {
      menuHTML += "<select disabled id='data-"+collection.fields[i].id+"' name='"+collection.fields[i].id+"'>";
      menuHTML += "<option value='true'"+(data[collection.fields[i].id] ? "selected" : "")+">True</option>";
      menuHTML += "<option value='false'"+(!data[collection.fields[i].id] ? "selected" : "")+">False</option>";
      menuHTML += "</select>";
    }
  ...
  menuHTML += "</ul>";
  menuHTML += "</div>";
  menuHTML += "<div id='poros-diffview'" + (window.localStorage.getItem('view') == 'diff' ? "class='poros-editor-hidden-tab'" : "") + ">";
    if (!isCurrentRevision) {
      var strategy = new ComparingKeyAndValueStrategy();
      const left = JSON.stringify(data);
      const right = JSON.stringify(currentVersion);
      var result = getDiffRepresentation(left, right, strategy);
      menuHTML += "<p>Diff</p>";
      menuHTML += "<pre>{\n";
      for(var i = 0; i < result.diff.length; i++) {
        if(result.diff[i].op === "NONE") menuHTML +="\t" + result.diff[i].key + ": ";
        if(result.diff[i].op === "ADD") menuHTML +="\t +" + result.diff[i].key + ": ";
        if(result.diff[i].op === "REMOVE") menuHTML +="\t -" + result.diff[i].key + ": ";
        menuHTML += result.diff[i].value;
        menuHTML += "\n";
      }
      menuHTML += "}</pre>";
    }
  ...
  if(!isCurrentRevision) menuHTML += "<button id='poros-button-okay' />Restore</button><button id='poros-button-cancel' />Cancel</button>";
  else menuHTML += "<button id='poros-button-cancel' />Cancel</button>";
  menuHTML += "<button id='poros-button-delete' />DELETE REVISION</button>";

  showRecordEdit(menuHTML);

  var subMenuBack = function(){ page('/poros-collections/'+collection.id+"/"+record+"/revisions") }
  var subMenuRestoreRevision = function() {
    if(confirm("Confirm?")) {
      const url = "/api/collections/"+collection.id+"/"+record+"/revisions/"
      enqueueData(url,
        "POST",
        {
          restoreId: revisionId
        },
        function() { page('/poros-collections/'+collection.id+"/"+record) }
      );
    }
  }
  var subMenuDeleteRevision = function() {
      // enqueueData(url,
      //   "DELETE",
      //   data,
      //   function() { subMenuBack(); }
  ...
  var okay = document.getElementById('poros-button-okay');
  if (okay) okay.addEventListener("click", subMenuRestoreRevision);
  var cancel = document.getElementById('poros-button-cancel');
  cancel.addEventListener("click", closeEditMenu);
  var del  = document.getElementById('poros-button-delete');
  del.addEventListener("click", subMenuDeleteRevision);
  var dataViewButton  = document.getElementById('poros-button-view');
  if (dataViewButton) dataViewButton.addEventListener("click", toggleView);

  function toggleView () {
    if(window.localStorage.getItem('view') === 'diff') {
      window.localStorage.setItem('view', 'object')
      dataViewButton.innerText = 'Object'
      var dataView  = document.getElementById('poros-dataview');
      var diffView  = document.getElementById('poros-diffview');
      addClass(dataView, 'poros-editor-hidden-tab')
      removeClass(diffView, 'poros-editor-hidden-tab')
    } else {
      window.localStorage.setItem('view', 'diff')
      dataViewButton.innerText = 'Diff'
      var dataView  = document.getElementById('poros-dataview');
      var diffView  = document.getElementById('poros-diffview');
      addClass(diffView, 'poros-editor-hidden-tab')
      removeClass(dataView, 'poros-editor-hidden-tab')
    }
  }
}
