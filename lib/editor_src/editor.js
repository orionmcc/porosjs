...
function showRecordsSubMenu (collection, records) {
  ...
  menuHTML += "<input type='file' id='recordFileImport' onChange=\"onFile.call(this, null, '/api/import/"+collection+"', function() { page('/poros-collections/"+collection+"') })\" accept='text/csv' style='display:none' />"
  menuHTML += "<h2 style='cursor:pointer;' onClick='page.redirect(\"/poros-collections\");'>ALL COLLECITONS</h2>";
...
function editPage(data, templates) {
  ...
  var subMenuUpdate = function() {
    ...
    var def = document.getElementById('data-default').value === 'true';
    ...
    if (slug && template) {
      ...
      enqueueData(url,
        "POST",
        "slug="+slug+"&template="+template+"&access="+access+"&def="+def,
        function() { subMenuBack(); }
      );
    }
  }
