...
export function handlePage(pageId, siteVars, paths, FX, COLLECTIONS, getTemplate, pageStrings) {
  return async function(req, res, next) {
    try {
      ...
      //Attach page vars
      let Page = {};
      let PAGEDATA = (await DB.databaseGet('pages', 'page', pageId))[0];
      ...
      for (let name of Template.getCollections()) {
        ...
        const displayMap =  FX[COLLECTIONS[name].displayMapping];
        collections[name] = displayMap ?
          (await DB.databaseGet('collections', name)).map(displayMap) :
          (await DB.databaseGet('collections', name));
      }
