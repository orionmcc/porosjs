...
export async function setCollectionRecord(name, rangeOne, attributes) {
  return DB.databaseGet('collections', name, rangeOne).then(m => {
      ...
      console.log('setCollectionRecord', name, rangeOne, { ...m, ...attributes, lastUpdatedAt: new Date()});
      return DB.databasePut('collections', name, rangeOne, { ...m, ...attributes, lastUpdatedAt: new Date()});
    }
  )
}
    ...
    try {
      ...
      for (let name of Template.getCollections()) {
        ...
        if (typeof name === "object") {
          ...
          collections[collectionDef["as"] || collectionDef["table"]] = await DB.databaseGet('collections', collectionDef["table"]);
        } else {
...
export function handleAPI(func, DB, EMAIL) {
  return async function(req, res, next) {
    ...
    try {
      ...
      response = await func.call(new PorosAPIContext(req, res, DB, EMAIL), req.body, req.params);
    } catch(err) {
