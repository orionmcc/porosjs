...
export function handlePage(pageId, siteVars, paths, getTemplate, getData) {
  return async function(req, res, next) {
    try {
      ...
      //     name: "eagle",
      //     img: "eagle",
      //     description: "The eagle is majestic",
      //   },
      //   {
      //     name: "baseball",
      //     img: "baseball",
      ...
      if (editable) {
        for (let n in SITEDATA[0]) {
          ...
          const type = v.type;
          const name = n[0] == '_' ? n.substr(1) : n
          console.log("SITEDATA", n, name, type, SITEDATA[0][n]);
      ...
      let composedTemplate = `<!DOCTYPE html><html><head>${styles}${scripts}${Template.getMeta().replace(/%{/g, "{{{").replace(/}%/g, "}}}")}</head><body>${Template.getBody().replace(/%{/g, "{{{").replace(/}%/g, "}}}")}</body></html>`;
