...
function getCollectionFunction(functionName, FX, COLLECTIONS, collection) {
    ...
    let collectionFunction = FX[collectionFunctions[functionName]] || (data => data);
    ...
    return templateCollectionFunctions.reduce((acc, f) => { return x => acc(f(x)) }, collectionFunction)
}
...
async function writeRecord (COLLECTIONS, collectionname, recordname, recordData) {
  ...
  return await DB.databasePut('collections', collectionname, recordname, data);
}
...
export function postRecord(COLLECTIONS, FX) {
  return async function(req, res, next) {
    ...
    const normalizedData = normalizeFunction(req.body);
    // beforeWriteFunction({ context: new PorosAPIContext(req, res, DB), collectionname, recordname, record: normalizedData});
    await writeRecord(COLLECTIONS, collectionname, recordname, normalizedData);
    // afterWriteFunction({ context: {
    //     DB,
    //     writeCurrentRecord: (data) => writeRecord(COLLECTIONS, collectionname, recordname, data)
    //   }, record: normalizedData});
    // //afterWriteFunction({ context: new PorosAPIContext(req, res, DB), collectionname, recordname, record: req.body});
...
export function importRecords(COLLECTIONS, FX) {
  return async function(req, res, next) {
    ...
    for (let i in records) {
      ...
      if (importedData) {
        const normalizedData = normalizeFunction(importedData.data);
        // beforeWriteFunction({ context: new PorosAPIContext(req, res, DB), collectionname, recordname: importedData.pid, record: normalizedData});
        await writeRecord(COLLECTIONS, collectionname, importedData.pid, normalizedData);
        // afterWriteFunction({ context: new PorosAPIContext(req, res, DB), collectionname, recordname: importedData.pid, record: normalizedData});
      }
