...
export function postPage(app, SITEVARS, TEMPLATES, FX, PAGES, reset){
  return async function(req, res, next) {
    let newId = 0;
    let {slug, template, access} = req.body;
    let id = req.params.id;
    if(id) id = Number(id);

    for (let i = 0; i < PAGES.length; i++) {
      if ( id && id == PAGES[i].id ) {
        console.log('updating page',id, 'with', slug, template, access);
        await DB.databasePut('pages', 'page', id, {slug, id, template, access});
        res.sendStatus(200);
        next();
...
export function postUser(){
  return async function(req, res, next) {
    let {role, newPassword, oldPassword} = req.body;
    let username = req.params.username;
