...
export function restoreRevision(COLLECTIONS, FX) {
  return async function(req, res, next) {
    ...
    //writeRecord(COLLECTIONS, collectionname, revision);
    writeRecord(COLLECTIONS, FX, collectionname, name, revision);

    res.sendStatus(200);
  }
}

export async function writeRecord (COLLECTIONS, FX, collectionname, recordName, rawData) {
  ...
  console.log('Write Record', collectionname, recordName, normalizedData);
  await DB.databasePut('collections', collectionname, recordName, normalizedData);

  console.log('Revision added', `${collectionname}.${recordName}`, (+new Date).toString(36));
  await DB.databasePut('revisions', `${collectionname}.${recordName}`, (+new Date).toString(36), normalizedData);
...
export function postRecord(COLLECTIONS, FX) {
  return async function(req, res, next) {
    ...
    await writeRecord(COLLECTIONS, FX, collectionname, recordname, req.body);
...
export function importRecords(COLLECTIONS, FX) {
  return async function(req, res, next) {
    ...
    for (let i in records) {
      ...
      if (imported) {
        //const normalizedData = normalizeFunction(importedData);
        //await writeRecord(COLLECTIONS, collectionname, normalizedData);
        await writeRecord(COLLECTIONS, FX, collectionname, imported.recordName, imported.data);
      }
