...
export function restoreRevision(COLLECTIONS) {
  return async function(req, res, next) {
    let collectionname = req.params.collection;
    let name = req.params.record;
...
export async function writeRecord (COLLECTIONS, collectionname, recordData) {
  ...
  await DB.databasePut('collections', collectionname, recordData.pid, data);

  console.log('Revision added', `${collectionname}.${recordData.pid}`, (+new Date).toString(36));
  await DB.databasePut('revisions', `${collectionname}.${recordData.pid}`, (+new Date).toString(36), data);
