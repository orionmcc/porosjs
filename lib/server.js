...
async function start(port, siteName)
{
    ...
    function serveSite(PAGES, SITEDATA, FX, STRINGS) {
      ...
      app.use(session({
        secret: process.env.SESSION_SECRET || 'a super secret',
        resave: false,
      ...
      app.post('/api/import/:collection/', Upload.single('Records'), importRecords(SITEDATA.collections, FX));
      app.get('/api/collections/:collection/:record', getRecord(SITEDATA.collections));
      app.post('/api/collections/:collection/:record', postRecord(SITEDATA.collections));
      ...
      for (let api of SITEDATA.apis) {
        ...
        console.log(`Adding api route ${api.method} /siteapi/${api.route}`);
        METHOD.call(app, `/siteapi/${api.route}`, ADMIN ? verifyAuthEndpoint('poros.admin') : (req, res, next) => next() , handleAPI(FX[FUNCTION], DB));
      }
