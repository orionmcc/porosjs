...
import {handleStaticAssets, handleLib, handlePage, handleAPI, runInitializer,
...
import {verifyAuthEndpoint, verifyAuthToken} from './authentication';
...
async function start(port, siteName)
{
    ...
    function serveSite(PAGES, SITEDATA, FX, STRINGS) {
      ...
      app.post(`/${PATHS.admin || 'admin'}`,  [authenticateUser(), function(req, res, next){ res.redirect('/#!'); }]);
      ...
      app.get('/lib/*', handleLib(__dirname + `/../${site}/`, SITEDATA.libs, `${PATHS.scripts || 'scripts'}/`));
          ...
          && (page.access === 'poros.maintenance')) {
            handlers.push(async function(req, res, next) {
              ...
              if (tokenVerified) {
                ...
                res.status(200).sendFile(path.join(__dirname, '/../', site, 'static/maintenance.html'));
              }
            });
        }
      ...
      app.use(function(req, res, next){
        ...
        if (req.accepts('html')) {
          res.status(404).sendFile(path.join(__dirname, '/../', site, 'static/404.html'));
          return;
        }
      ...
      for (let initializer of SITEDATA.initializers) {
