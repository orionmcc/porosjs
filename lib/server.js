...
import {handleStaticAssets, handleLib, handlePage, handleAPI, authenticateUser} from './core';
import {postData, getSiteData, getPageData, getTemplates, getPages, postPage} from './api';
...
async function start(port, siteName)
{
    ...
    function serveSite(PAGES, SITEDATA, FX) {
      ...
      app.get('/api/data/site', getSiteData(SITEDATA.site));
      app.get('/api/data/site/templates/', getTemplates(SITEDATA.templates));

      app.get('/api/data/pages/', getPages(PAGES));
      app.get('/api/data/page/:id', getPageData(PAGES, SITEDATA.templates));
      app.post('/api/data/page/:id', postPage(app, SITEDATA.site, SITEDATA.templates, FX, PAGES, resetServer));
