...
async function start(port, siteName)
{
    ...
    function serveSite(PAGES, SITEDATA, FX, STRINGS) {
      ...
      app.post('/api/collections/:collection/:record/revisions', restoreRevision(SITEDATA.collections));
      ...
      for (let api of SITEDATA.apis) {
        ...
        METHOD.call(app, PATH, ADMIN ? verifyAuthEndpoint('poros.admin') : (req, res, next) => next() , handleAPI(FX[FUNCTION], DB, EMAIL));
      }
